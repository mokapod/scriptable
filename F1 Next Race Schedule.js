...
const WIDGET_LAYOUT = {
    ...
    spacing: {
        rows: { lock: 2, small: 10, default: 4 },
        columns: 0
    },
    background: {
        widget: {
        ...
        title: {
            gradient: {
                locations: [0, 0.87, 0.98],
                colors: [
                    new Color("#E8002D", 0.85),   // F1 red with transparency
                    new Color("#E8002D"),        // Solid F1 red
                    new Color("#E8002D", 0.2)   // F1 red with transparency
                ],
...
async function createWidget() {
    ...
    // Render subheader
    renderSubheader(listWidget, race, options);
...
function renderSubheader(listWidget, race, options) {
    // Race Circuit
    const circuitStack = listWidget.addStack();
    const circuitNameText = race.Circuit.circuitName;
    const circuitNameCell = circuitStack.addStack();
    circuitNameCell.size = new Size(options.width, 0);
    circuitNameCell.addSpacer();
    const circuitNameElement = circuitNameCell.addText(circuitNameText);
    circuitNameElement.font = new Font(...options.font.body);
    circuitNameElement.minimumScaleFactor = 0.5;
    circuitNameElement.lineLimit = 1;
    circuitNameCell.addSpacer();
    
    listWidget.addSpacer(options.spaceBetweenRows);

    // Race Weekend Dates
    const datesStack = listWidget.addStack();
    const fp1 = race.FirstPractice;
    const raceStartDate = new Date(`${fp1.date}T00:00:00`);
    const raceEndDate = new Date(`${race.date}T00:00:00`);
    const raceDatesText = formatRaceWeekendDates(raceStartDate, raceEndDate);

    const raceDatesCell = datesStack.addStack();
    raceDatesCell.size = new Size(options.width, 0);
    raceDatesCell.addSpacer();
    const raceDatesElement = raceDatesCell.addText(raceDatesText);
    raceDatesElement.font = new Font(...options.font.body);
    raceDatesElement.minimumScaleFactor = 0.5;
    raceDatesElement.lineLimit = 1;
    raceDatesCell.addSpacer();
